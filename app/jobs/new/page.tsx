...
import { deployWalletEscrow, sameAddress } from "@/lib/wallet-client";
...
export default function PostJobPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (wallet.mode === "wallet" && !/^0x[0-9a-fA-F]{40}$/.test(workerAddress.trim())) return setError("Enter the worker’s valid Bradbury wallet address.");
    if (wallet.mode === "wallet" && sameAddress(wallet.address, workerAddress.trim())) return setError("The client and worker must use different wallet addresses.");
    ...
    try {
      const common = { title: title.trim() || null, spec: spec.trim(), fee_bps: Math.round(feeNumber * 100), min_score: minNumber, partial_floor: floorNumber };
      let response;
      if (wallet.mode === "wallet") {
        const sourceResponse = await fetch("/api/contract-source", { cache: "no-store" });
        if (!sourceResponse.ok) throw new Error("The verified escrow contract source could not be loaded.");
        const deployment = await deployWalletEscrow({
          account: wallet.address!, code: await sourceResponse.text(), spec: common.spec,
          worker: workerAddress.trim(), platform: demo!.platform_address!, feeBps: common.fee_bps,
          minScore: common.min_score, partialFloor: common.partial_floor,
        });
        const pending = { ...common, address: deployment.address, client_address: wallet.address!, worker_address: workerAddress.trim(), deployment_tx: deployment.hash };
  ...
  const continueBuilder = () => {
    ...
    if (step === 1 && wallet.mode === "wallet" && !/^0x[0-9a-fA-F]{40}$/.test(workerAddress.trim())) return setError("Enter the worker’s valid Bradbury wallet address before continuing.");
    if (step === 1 && wallet.mode === "wallet" && sameAddress(wallet.address, workerAddress.trim())) return setError("The client and worker must use different wallet addresses.");
    if (step === 2) {
