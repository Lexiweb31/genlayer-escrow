...
export default function BrowseJobsPage() {
  ...
  return <div className="page-container">
    ...
    <section className="market-toolbar" aria-label="Marketplace filters"><label className="search-field"><span>Search</span><input type="search" value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder="Title, specification, or address"/></label><label><span>Status</span><select value={status} onChange={(event) => { setStatus(event.target.value); setPage(1); }}><option value="ALL">All jobs</option><option value="ACTIVE">Active lifecycle</option><option value="SETTLEMENT_PENDING">Settlement pending</option><option value="FINALIZED">Finalized</option><option value="LEGACY_UNSAFE">Legacy read-only</option></select></label><label><span>Sort</span><select value={sort} onChange={(event) => { setSort(event.target.value); setPage(1); }}><option value="NEWEST">Newest first</option><option value="OLDEST">Oldest first</option><option value="VALUE_HIGH">Highest GEN value</option><option value="VALUE_LOW">Lowest GEN value</option></select></label></section>
    {loading && !data
