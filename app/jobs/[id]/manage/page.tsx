...
export default function ManageJobPage() {
  ...
  const actionPanel = () => {
    ...
    if (view.isPending) return <div className="action-copy"><StatusPill tone="warning"><ClockIcon size={14}/> {view.hasTransactionReference ? "Payment processing" : "Transaction unverified"}</StatusPill><h2>{view.hasTransactionReference ? "Payment transaction submitted" : view.label}</h2><p>{view.hasTransactionReference ? "The transaction is saved. You can safely leave this page and return later for confirmed payment evidence." : "The contract reports a pending settlement, but no transaction reference is available. Recover it using the transaction hash from the client wallet."}</p><small className="technical-status">Technical status · {view.hasTransactionReference ? "Settlement pending" : "Missing transaction reference"}</small>{!view.hasTransactionReference && wallet.mode === "wallet" && isClient && <form onSubmit={recoverSettlement}><label><span>Settlement transaction hash</span><input value={recoveryTx} onChange={(event) => setRecoveryTx(event.target.value)} placeholder="0x…" autoComplete="off" spellCheck={false}/></label><button className="button primary large" disabled={busy}>Verify and recover payment</button></form>}<button className="button secondary large" disabled={loading} onClick={refresh}><RefreshIcon/> Refresh payment status</button></div>;
  ...
  return <div className="page-container manage-page">
    <JobNavigation id={id}/><PageHeader eyebrow="Manage protected job" title={data.meta.title || "Protected job"} description="Complete only the action assigned to the connected wallet. Merit verifies every confirmed step from funding through settlement." actions={<StatusPill tone={visiblePendingAction || view.isPending ? "warning" : view.isFinalized ? "success" : "info"}>{visiblePendingAction ? "Transaction processing" : view.label}</StatusPill>}/>
    <section className="role-lanes" aria-label="Contract-separated client and worker roles"><article className={clientLaneActive ? "active" : ""}><span><UserIcon/></span><div><small>{wallet.mode === "wallet" ? "Client wallet" : "Demo client lane"}</small><b>Deploy · fund · evaluate · finalize</b><p title={clientAddress}>{clientAddress || "Client address unavailable"}</p></div>{clientLaneActive ? <StatusPill tone={clientCanSign ? "info" : "warning"}>{clientCanSign ? wallet.mode === "wallet" ? "Connected" : "Current signer" : "Client required"}</StatusPill> : <StatusPill>Waiting</StatusPill>}</article><div className="role-divider"><LockIcon/><span>Contract-enforced separation</span></div><article className={workerLaneActive ? "active" : ""}><span><UserIcon/></span><div><small>{wallet.mode === "wallet" ? "Worker wallet" : "Demo worker lane"}</small><b>Accept · submit public work</b><p title={workerAddress}>{workerAddress || "Worker address unavailable"}</p></div>{workerLaneActive ? <StatusPill tone={workerCanSign ? "success" : "warning"}>{workerCanSign ? wallet.mode === "wallet" ? "Connected" : "Current signer" : "Worker required"}</StatusPill> : <StatusPill>Waiting</StatusPill>}</article></section>
