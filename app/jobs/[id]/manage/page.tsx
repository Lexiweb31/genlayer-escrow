...
export default function ManageJobPage() {
  ...
  const actionPanel = () => {
    ...
    if (job.status === "OPEN") return <div className="action-copy"><p className="step-label">Step 2 · Demo worker</p><h2>Accept the job requirements</h2><p>The separate demo worker confirms the public requirements before starting work.</p><button className="button primary large" disabled={busy || !live} onClick={() => runAction("Accepting requirements", () => meritApi.acceptTerms(id))}>{busy ? "Submitting…" : "Accept with demo worker"}<ArrowIcon/></button></div>;
    if (job.status === "AGREED") return <form onSubmit={submit}><p className="step-label">Step 3 · Demo worker</p><h2>Submit the finished work</h2><p>Provide the public URL that AI validators should compare with the job requirements.</p><label><span>Public link to the finished work</span><input type="url" value={submissionUrl} onChange={(event) => setSubmissionUrl(event.target.value)} placeholder="https://example.com/deliverable"/></label><button className="button primary large" disabled={busy || !live}>{busy ? "Submitting…" : "Submit with demo worker"}<ArrowIcon/></button></form>;
    ...
    if (job.status === "EVALUATED") return <div className="action-copy"><p className="step-label">Step 5 · Demo client</p><h2>Submit the payment transaction</h2><p>The evaluation result is ready. Payment will remain “processing” until the on-chain transfer is confirmed.</p><details className="score-help"><summary>Technical details</summary><p>The contract decision is recorded first, then an outbound transfer is queued for on-chain finalization.</p></details><button className="button primary large" disabled={busy || !live} onClick={() => runAction("Submitting payment", () => meritApi.finalize(id))}>{busy ? "Submitting…" : "Submit payment with demo client"}<ArrowIcon/></button></div>;
    if (view.isPending) return <div className="action-copy"><StatusPill tone="warning"><ClockIcon size={14}/> Payment processing</StatusPill><h2>Payment transaction submitted</h2><p>Refresh safely to check whether the transfer has been confirmed. Do not submit it again while it is processing.</p><small className="technical-status">Technical status · Settlement pending</small><button className="button secondary large" disabled={loading} onClick={refresh}><RefreshIcon/> Refresh payment status</button></div>;
