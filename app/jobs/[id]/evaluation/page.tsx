...
export default function EvaluationPage() {
  ...
  const isBounty = job.job_type === "BOUNTY";
  ...
  const isParty = isClient || sameAddress(wallet.address, workerAddress) || Boolean(job.submissions?.some((entry) => sameAddress(entry.submitter, wallet.address)));
  ...
  if (isBounty) return <div className="page-container">
    <JobNavigation id={id}/><PageHeader eyebrow="Bounty intelligence" title="Rank every entry against one standard" description="GenLayer evaluates every public candidate, records the strongest score, and applies deterministic tie-breaking." actions={<button className="button secondary" onClick={refresh} disabled={loading}><RefreshIcon/> Refresh</button>}/>
    <section className="panel bounty-evaluation-room"><div className="panel-heading"><div><p className="card-kicker">{job.submission_count || job.submissions?.length || 0} verified entries</p><h2>{evaluated ? "Bounty ranking recorded" : job.status === "CLOSED" ? "Submissions are closed and ready" : "Waiting for the client to close entries"}</h2></div>{evaluated && <StatusPill tone="success"><CheckIcon/> Evaluation completed</StatusPill>}</div>
      <div className="bounty-entry-grid">{(job.submissions || []).map((entry) => <article key={entry.position} className={sameAddress(entry.submitter, job.winner || data.result.winner) ? "winner" : ""}><span>Entry {entry.position}</span><b>{sameAddress(entry.submitter, job.winner || data.result.winner) ? "Winning submission" : shortAddress(entry.submitter)}</b><a href={entry.url} target="_blank" rel="noreferrer">Inspect work <ExternalIcon/></a>{evaluated && <><strong>{entry.score ?? 0}/100</strong><p>{entry.reasoning || "Evaluation evidence recorded on-chain."}</p></>}</article>)}</div>
      {!evaluated && job.status === "CLOSED" && <button className="button primary large" onClick={runEvaluation} disabled={busy || Boolean(pendingEvaluationHash) || !live || (wallet.mode === "wallet" && !isClient)}>{busy || pendingEvaluationHash ? "Validators are evaluating…" : isClient || wallet.mode === "demo" ? "Evaluate every entry" : "Client wallet required"}<ArrowIcon/></button>}
      {evaluated && <div className="evaluation-consequence outcome-success"><span>Strongest result</span><strong>{score}/100 · {score >= (job.min_score ?? 70) ? "qualifies for winner payout" : "no entry reached the qualifying score"}</strong><p>{data.result.reasoning}</p><Link className="button primary" href={`/jobs/${encodeURIComponent(id)}/manage`}>Continue to settlement <ArrowIcon/></Link></div>}
      {message && <div className="action-message info" role="status">{message}</div>}
    </section>
  </div>;
  return <div className="page-container">
    <JobNavigation id={id}/><PageHeader eyebrow={isBounty ? "Bounty intelligence" : "AI Evaluation"} title={isBounty ? "Rank every entry against one standard" : "Check the work against the requirements"} description={isBounty ? "GenLayer evaluates every candidate, records the strongest score, and applies deterministic tie-breaking." : "Merit shows a score only after the backend confirms the evaluation and reasoning."} actions={<button className="button secondary" onClick={refresh} disabled={loading}><RefreshIcon/> Refresh</button>}/>
