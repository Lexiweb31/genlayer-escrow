...
export default function JobOverviewPage() {
  ...
  const hasSubmitted = isBounty && submissions.some((entry) => sameAddress(entry.submitter, wallet.address));
  const viewerRole = wallet.mode === "demo" ? "demo" : isClient ? "client" : isWorker ? "worker" : hasSubmitted ? "entrant" : "visitor";
  const nextHref = view.isFinalized
    ? `/jobs/${encodeURIComponent(id)}/evaluation`
    : view.isPending
      ? `/jobs/${encodeURIComponent(id)}/manage`
        : (job.status === "SUBMITTED" || (isBounty && job.status === "CLOSED")) && viewerRole !== "worker"
        ? `/jobs/${encodeURIComponent(id)}/evaluation`
        : `/jobs/${encodeURIComponent(id)}/manage`;
    ...
    : view.isPending
      ...
      : viewerRole === "client"
        ...
        : viewerRole === "worker"
          ...
          : viewerRole === "entrant"
            ? "Track your Bounty entry"
          : viewerRole === "visitor"
            ? "Public view only"
            : "Open next action";
  return <div className="page-container">
    <JobNavigation id={id}/>
    <PageHeader eyebrow={viewerRole === "client" ? isBounty ? "Your client Bounty" : "Your client job" : viewerRole === "worker" ? isBounty ? "Your winning Bounty entry" : "Your assigned work" : viewerRole === "entrant" ? "Your Bounty entry" : isBounty ? "Public Bounty" : "Public protected job"} title={data.meta.title || "Protected job"} description={isBounty ? viewerRole === "client" ? "Track the open submission round, GenLayer ranking, winner, and verified settlement." : viewerRole === "worker" ? "Your entry ranked first. Track the confirmed evaluation and winner payout." : viewerRole === "entrant" ? "Track your recorded entry, the shared evaluation, and the final result." : data.meta.spec || job.spec || "Open competition with contract-enforced settlement." : viewerRole === "worker" ? "Track the requirements you accepted, your public submission, evaluation result, and verified payout." : viewerRole === "client" ? "Track protected funding, worker delivery, evaluation, and final settlement." : data.meta.spec || job.spec || "Job requirements unavailable."} actions={<button className="button secondary" onClick={refresh} disabled={loading}><RefreshIcon/> Refresh state</button>}/>
    ...
    <section className="overview-grid">
      <article className="panel overview-summary"><div className="panel-heading"><div><p className="card-kicker">Current state</p><h2>{view.label}</h2></div><StatusPill tone={view.isPending ? "warning" : view.isFinalized ? "success" : job.legacy_contract ? "danger" : "info"}>{view.status}</StatusPill></div><div className="amount-display"><span>{view.isFinalized ? "Settled value" : isBounty ? "Protected reward" : "Protected payment"}</span><strong>{formatWei(displayEscrowAmountWei(job))}</strong><small>{view.isPending ? "Settlement transaction submitted · awaiting confirmation" : view.isFinalized ? "Original escrow value distributed through the confirmed settlement" : isBounty ? `${submissions.length}/${job.max_submissions || 5} Bounty entries recorded` : "Held according to the current job step"}</small></div><div className="summary-grid"><div><span>{isBounty ? "Winning score" : "AI evaluation"}</span><strong>{scoreConfirmed ? `${data.result.score ?? job.score}/100` : "Not evaluated yet"}</strong></div><div><span>{isBounty ? "Winner qualifies at" : "Full payment starts at"}</span><strong>{job.min_score ?? 70}/100</strong></div><div><span>{isBounty ? "Submission limit" : "Refund below"}</span><strong>{isBounty ? `${job.max_submissions || 5} entries` : `${job.partial_floor ?? 40}/100`}</strong></div><div><span>Merit fee</span><strong>{((job.fee_bps ?? 0) / 100).toFixed(2).replace(/\.00$/, "")}%</strong></div></div></article>
    ...
    {isBounty ? <section className="role-action-strip" aria-label="Bounty roles and next action"><article className={isClient ? "active" : ""}><span><UserIcon/></span><div><small>{isClient ? "You are the Bounty client" : "Bounty client"}</small><b>{shortAddress(clientAddress)}</b><p>Funds, closes entries, evaluates, and settles</p></div></article><article className={viewerRole === "entrant" || isWorker ? "active" : ""}><span><UserIcon/></span><div><small>{isWorker ? "You submitted the winning entry" : viewerRole === "entrant" ? "Your entry is recorded" : "Contributor pool"}</small><b>{submissions.length}/{job.max_submissions || 5} entries</b><p>One public submission per independent wallet</p></div></article><Link className="button primary" href={nextHref}>{nextLabel} <ArrowIcon/></Link></section> : <section className="role-action-strip" aria-label="Role-aware next actions"><article className={isClient ? "active" : ""}><span><UserIcon/></span><div><small>{isClient ? "You are the client" : "Client wallet"}</small><b>{shortAddress(clientAddress)}</b><p>{isClient ? ["UNFUNDED", "EVALUATED"].includes(job.status) ? "Your action is available" : "Waiting for the worker or contract" : "Funds, evaluates, and settles"}</p></div></article><article className={isWorker ? "active" : ""}><span><UserIcon/></span><div><small>{isWorker ? "You are the worker" : "Worker wallet"}</small><b>{shortAddress(workerAddress)}</b><p>{isWorker ? ["OPEN", "AGREED"].includes(job.status) ? "Your action is available" : "No worker action required now" : "Accepts and submits public work"}</p></div></article>{viewerRole === "visitor" ? <span className="button secondary" aria-disabled="true">Connect assigned wallet</span> : <Link className="button primary" href={nextHref}>{nextLabel} <ArrowIcon/></Link>}</section>}
    <SettlementProof job={job} result={data.result}/>
    <section className="overview-detail-grid"><article className="panel parties-panel"><div className="panel-heading"><div><p className="card-kicker">People and work</p><h2>{isBounty ? "Client and Bounty entries" : "Client and worker record"}</h2></div></div><div className="party-list"><div><UserIcon/><span>Client</span><MonoValue>{job.client_address || job.client || data.demo.client_address || "Unavailable"}</MonoValue></div>{isBounty ? submissions.map((entry) => <div className="bounty-party-row" key={entry.position}><UserIcon/><span>Entry {entry.position}</span><MonoValue>{entry.submitter}</MonoValue><a href={entry.url} target="_blank" rel="noreferrer">Open work <ExternalIcon size={14}/></a></div>) : <><div><UserIcon/><span>Worker</span><MonoValue>{job.worker_address || job.worker || data.demo.worker_address || "Unavailable"}</MonoValue></div><div><FileIcon/><span>Finished work</span>{job.submission_url ? <a href={job.submission_url} target="_blank" rel="noreferrer">Open public work <ExternalIcon size={14}/></a> : <strong>Not submitted yet</strong>}</div></>}</div></article><article className="panel activity-panel"><div className="panel-heading"><div><p className="card-kicker">Job activity</p><h2>History</h2></div></div><ol><li><ClockIcon/><div><b>{isBounty ? "Bounty published" : "Job published"}</b><small>{relativeTime(job.created_at) || "Timestamp unavailable"}</small></div></li>{job.deployment_tx && <li><ShieldIcon/><div><b>Contract transaction recorded</b><MonoValue>{shortAddress(job.deployment_tx, 10, 7)}</MonoValue></div></li>}{isBounty ? submissions.length > 0 && <li><FileIcon/><div><b>{submissions.length} Bounty {submissions.length === 1 ? "entry" : "entries"} recorded</b><small>Public submissions available for inspection</small></div></li> : job.submission_url && <li><FileIcon/><div><b>Work submitted</b><small>Available for inspection</small></div></li>}<li><ClockIcon/><div><b>Last update</b><small>{relativeTime(job.updated_at) || "Timestamp unavailable"}</small></div></li></ol></article></section>
