...
export default function JobOverviewPage() {
  ...
  return <div className="page-container">
    <JobNavigation id={id}/>
    <PageHeader eyebrow="Protected job overview" title={data.meta.title || "Protected job"} description={data.meta.spec || job.spec || "Job requirements unavailable."} actions={<button className="button secondary" onClick={refresh} disabled={loading}><RefreshIcon/> Refresh state</button>}/>
    ...
    <section className="overview-grid">
      <article className="panel overview-summary"><div className="panel-heading"><div><p className="card-kicker">Current state</p><h2>{view.label}</h2></div><StatusPill tone={view.isPending ? "warning" : view.isFinalized ? "success" : job.legacy_contract ? "danger" : "info"}>{view.status}</StatusPill></div><div className="amount-display"><span>Protected payment</span><strong>{formatWei(job.amount)}</strong><small>{view.isPending ? "Payment transaction submitted · awaiting confirmation" : view.isFinalized ? "Payment confirmed" : "Held according to the current job step"}</small></div><div className="summary-grid"><div><span>AI evaluation</span><strong>{scoreConfirmed ? `${data.result.score ?? job.score}/100` : "Not evaluated yet"}</strong></div><div><span>Full payment starts at</span><strong>{job.min_score ?? 70}/100</strong></div><div><span>Refund below</span><strong>{job.partial_floor ?? 40}/100</strong></div><div><span>Merit fee</span><strong>{((job.fee_bps ?? 0) / 100).toFixed(2).replace(/\.00$/, "")}%</strong></div></div></article>
    ...
    <section className="role-action-strip" aria-label="Role-aware next actions"><article><span><UserIcon/></span><div><small>{wallet.mode === "wallet" ? "Client wallet" : "Demo client"}</small><b>{shortAddress(clientAddress)}</b><p>{["UNFUNDED", "EVALUATED"].includes(job.status) ? "Action available in Manage Job" : "Waiting for the worker or contract state"}</p></div></article><article><span><UserIcon/></span><div><small>{wallet.mode === "wallet" ? "Worker wallet" : "Demo worker"}</small><b>{shortAddress(workerAddress)}</b><p>{["OPEN", "AGREED"].includes(job.status) ? "Action available in Manage Job" : "Waiting for the client or contract state"}</p></div></article><Link className="button primary" href={job.status === "SUBMITTED" ? `/jobs/${encodeURIComponent(id)}/evaluation` : `/jobs/${encodeURIComponent(id)}/manage`}>{job.status === "SUBMITTED" ? "Open evaluation" : "Open next action"} <ArrowIcon/></Link></section>
