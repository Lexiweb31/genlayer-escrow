...
export default function HomePage() {
  ...
  return <div className="home-page">
    ...
    <section className="lab-section">
      ...
      <div className="lab-card"><div className="lab-card-head"><span>Simulated validator score</span><b>{labScore}<small>/100</small></b></div><input aria-label="Simulated validator score" type="range" min="0" max="100" value={labScore} onChange={(event) => setLabScore(Number(event.target.value))}/><div className="range-labels"><span>0</span><span>40</span><span>70</span><span>100</span></div><div className="lab-result"><StatusPill tone={result.tone}>{result.label}</StatusPill><strong>{result.detail}</strong><small>Simulation only · no on-chain transaction</small></div></div>
    </section>
