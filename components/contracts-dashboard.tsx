...
export function ContractsDashboard({ jobs, addresses }: { jobs: JobRecord[]; addresses?: ContractAddresses }) {
  const addressRows: Array<[string, string | undefined, string | null | undefined]> = addresses ? [
  ...
  return <div className="contracts-stack">
    <section className="infrastructure-grid">
      <article className="panel infrastructure-card"><span className="feature-icon"><ContractIcon/></span><div><p className="card-kicker">Contract layer</p><h2>Freelance escrow</h2><p>Immutable roles, thresholds, evaluator calls, and safe external-message settlement.</p><a className="text-link" href="https://github.com/Lexiweb31/genlayer-escrow/blob/main/contracts/freelance_escrow.py" target="_blank" rel="noreferrer">Inspect contract source <ExternalIcon size={13}/></a></div><StatusPill tone="success">Safe EOA transfer v2</StatusPill></article>
    ...
    {addresses && <section className="panel address-panel"><div className="panel-heading"><div><p className="card-kicker">Selected escrow infrastructure</p><h2>Deployed addresses</h2></div></div><div className="address-list">{addressRows.map(([label, value, url]) => value && <div key={label}><span>{label}</span><MonoValue title={value}>{value}</MonoValue><CopyButton value={value} label={label}/>{url && <a href={url} target="_blank" rel="noreferrer" aria-label={`Open ${label} in explorer`}><ExternalIcon/></a>}</div>)}</div></section>}
