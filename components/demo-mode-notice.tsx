...
export function DemoModeNotice({ compact = false }: { compact?: boolean }) {
  ...
  if (wallet.mode === "wallet") return <aside className="demo-notice wallet-notice" aria-label="Wallet mode notice">
    ...
    <div className="demo-copy">
      ...
      <details className="demo-technical"><summary>Wallet Mode status</summary><div>Connected wallets can sign actions for existing contracts when their address matches the required client or worker role. Wallet-created job deployment is still locked until registry registration and worker assignment are verified. The app never falls back to a demo signer in Wallet Mode.</div></details>
