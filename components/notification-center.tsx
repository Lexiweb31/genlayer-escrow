...
const ITEMS_KEY = "merit-notifications-v1";
const SNAPSHOT_KEY = "merit-notification-snapshot-v1";
function readItems(): MeritNotification[] { try { return JSON.parse(localStorage.getItem(ITEMS_KEY) || "[]") as MeritNotification[]; } catch { return []; } }
function readSnapshot(): Record<string, JobStatus> { try { return JSON.parse(localStorage.getItem(SNAPSHOT_KEY) || "{}") as Record<string, JobStatus>; } catch { return {}; } }
...
export function NotificationCenter() {
  ...
  useEffect(() => { const timer = window.setTimeout(() => setItems(readItems()), 0); return () => window.clearTimeout(timer); }, []);
  ...
  }, [save, wallet.address, wallet.mode]);
