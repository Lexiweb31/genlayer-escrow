...
export function Lifecycle({ job, result, transactionPending = false }: { job: JobRecord; result?: EvaluationResult; transactionPending?: boolean }) {
  ...
  const stages = job.job_type === "BOUNTY"
    ? ["Bounty created", "Reward locked", "Entries open", "Submissions closed", "All entries evaluated", settlementStage, presentation.decision === "REFUNDED" ? "Reward refunded" : "Winner paid"]
    : ["Job created", "Payment locked", "Requirements accepted", "Work submitted", "Evaluation completed", settlementStage, confirmedLabel];
