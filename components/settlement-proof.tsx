...
export function SettlementProof({ job, result }: { job: JobRecord; result?: EvaluationResult }) {
  ...
  return <section className={`panel settlement-panel ${outcomeClass}`}>
    ...
    {settlement.parent_transaction && <div className="parent-proof"><span>Payment transaction reference</span><MonoValue title={settlement.parent_transaction}>{settlement.parent_transaction}</MonoValue>{parentLink && <a href={parentLink} target="_blank" rel="noreferrer">Inspect on Bradbury <ExternalIcon size={14}/></a>}</div>}</details>
  </section>;
}
